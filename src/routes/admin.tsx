...
export const Route = createFileRoute("/admin")({
  head: () => ({
    meta: [
      ...
      {
        name: "description",
        content: "Painel privado dos noivos para gerir RSVPs e mensagens dos convidados.",
      },
      { property: "og:title", content: "Admin · Joana & Diogo" },
      {
        property: "og:description",
        content: "Painel privado dos noivos para gerir RSVPs e mensagens.",
      },
...
function AdminPage() {
  ...
  const [tab, setTab] = useState<
    "rsvps" | "convidados" | "mesas" | "listas" | "comunicacoes" | "mensagens" | "avisos"
  >("rsvps");
  ...
  const counts = useMemo(() => {
    ...
    // Ignora "nenhuma", "nada" e afins — ver src/lib/rsvp-lists.ts
    const restrictions = listaRestricoes(rsvps as never).length;
    const restrictionPeople = pessoasComRestricao(rsvps as never);
    return { all: rsvps.length, yes, no, restrictions, restrictionPeople };
  }, [rsvps]);
  ...
  const filtered = useMemo(() => {
    return rsvps.filter((r) => {
      ...
      if (filter === "restrictions" && semConteudo(r.allergies)) return false;
      if (search) {
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6">
        ...
        <div className="max-w-md text-center">
          ...
          <p className="text-foreground/70 mb-6">Esta conta não tem acesso de administrador.</p>
  ...
  const filterChips: {
    ...
    { key: "no", label: "Não", count: counts.no, activeBg: "#B85C5C", activeText: "#fff" },
    {
      ...
      count: counts.restrictions,
      activeBg: "var(--gold, #C9A961)",
      activeText: "#fff",
    },
  ];
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-1 text-sm">
                          {r.email && (
                            <div className="flex items-center gap-2 text-foreground/80 min-w-0">
                              ...
                              <a href={`mailto:${r.email}`} className="hover:text-primary truncate">
                                {r.email}
                              </a>
                            </div>
                          )}
                        ...
                        {r.message && r.message.trim() && (
                          <div
                            ...
                            style={{
                              background:
                                "color-mix(in oklab, var(--gold, #C9A961) 8%, transparent)",
                              border:
                                "1px solid color-mix(in oklab, var(--gold, #C9A961) 50%, transparent)",
                            }}
                          >
                            <MessageCircleHeart
              ...
              <div className="space-y-3">
                {toDelete && (
                  <div className="bg-muted/50 border border-border p-3 text-sm text-foreground space-y-1">
                    ...
                    <p>
                      <span className="text-muted-foreground">Email:</span> {toDelete.email || "—"}
                    </p>
                    <p>
                      <span className="text-muted-foreground">Presença:</span>{" "}
                      {toDelete.attending ? "Sim" : "Não"}
                    </p>
                    <p>
                      <span className="text-muted-foreground">Pessoas:</span> {toDelete.guests}
                    </p>
                  </div>
                )}
      ...
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Pessoas">
              <Input
                ...
                value={draft.guests}
                onChange={(e) =>
                  setDraft({ ...draft, guests: Math.max(1, Number(e.target.value) || 1) })
                }
              />
            </Field>
          ...
          <Field label="Música favorita">
            <Input
              value={draft.song_suggestion ?? ""}
              onChange={(e) => setDraft({ ...draft, song_suggestion: e.target.value })}
            />
          </Field>
