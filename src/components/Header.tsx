...
import { X, Home, MapPin, Camera, CalendarCheck, MoreHorizontal, Share2, Smartphone, Share, Plus, Lock } from "lucide-react";
...
const WA_SHARE_URL = `https://wa.me/?text=${encodeURIComponent(
  "Joana & Diogo - 19 Setembro 2026 🌿 " + "https://joanaediogo.com"
)}`;

const links = [
  { id: "inicio", key: "tab.home" as const, icon: <Home className="w-4 h-4" strokeWidth={1.5} /> },
  { id: "local", key: "tab.place" as const, icon: <MapPin className="w-4 h-4" strokeWidth={1.5} /> },
  { id: "fotos", key: "tab.photos" as const, icon: <Camera className="w-4 h-4" strokeWidth={1.5} /> },
  { id: "rsvp", key: "tab.rsvp" as const, icon: <CalendarCheck className="w-4 h-4" strokeWidth={1.5} /> },
  { id: "mais", key: "tab.more" as const, icon: <MoreHorizontal className="w-4 h-4" strokeWidth={1.5} /> },
];
...
export function Header() {
  ...
  useEffect(() => {
    const sync = () => setActive((window.location.hash || "#inicio").slice(1));
    sync();
      ...
      >
        ...
        <nav className="drawer-nav">
          <div className="drawer-group">
            ...
            >
              <span className="drawer-item-icon"><MapPin className="w-[18px] h-[18px]" strokeWidth={1.5} /></span>
              <span className="drawer-item-text">{t("nav.event")}</span>
            </a>
          </div>
        </nav>
        ...
        <div className="drawer-actions">
          ...
          <button
            ...
            className="drawer-action"
          >
            <span className="drawer-item-icon"><Lock className="w-[18px] h-[18px]" strokeWidth={1.5} /></span>
            <span className="drawer-item-text">{t("nav.private")}</span>
          </button>
        </div>
